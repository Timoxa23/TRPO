...
void addNewPatients(struct Patients *a, int *size)
{
    ...
    system("pause");
}void individualTask(struct Patients *a, int *sizePatients) //Индивидуальное задание.
{
    int age, stop=0, n, i, maxNum=15;
    char diagnos[maxNum];
    outputInformationAboutPatients(a, sizePatients);
    array_nulling(diagnos, &maxNum);
    printf("Enter the age above which you want to withdraw patients: ");
    scanf("%d", &age);
    printf("Enter a diagnos: ");
    scanf("%s", &diagnos);
    for (i=0; i<*sizePatients; i++)
    {
        if (strcmp((a+i)->Diagnos, diagnos)==0 && (age<2022-(((a+i)->DateofBirthday)%10000)))
        {
